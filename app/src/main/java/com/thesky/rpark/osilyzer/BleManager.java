...
public class BleManager {
    ...
    // Debugging
    private static final String TAG = "PowerBalanceBleManager";
...
//		mContext.unregisterReceiver(mReceiver);
    ...
    public boolean scanLeDevice(final boolean enable, UUID[] uuid) {
        ...
        if (enable) {
            ...
            if(mState < STATE_CONNECTING) {
                Log.d(TAG, "# scanLeDevice - uuid - disable.");
                mState = STATE_IDLE;
                mHandler.obtainMessage(MESSAGE_STATE_CHANGE, STATE_IDLE, 0).sendToTarget();
            }
            stopScanning();
    ...
    private final BluetoothGattCallback mGattCallback = new BluetoothGattCallback() {
        ...
        public void onCharacteristicChanged(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic) {
            ...
            if (data != null && data.length > 0) {
                ...
                //}
                Log.d(TAG, "bytes:"+data.length + " data: " + new String(data));
